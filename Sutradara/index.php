...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "New director created successfully";
    } else {
...
<div class="w-full min-h-screen flex justify-center py-8">
    ...
    <div class="bg-white w-full max-w-md p-8 shadow-lg rounded-lg mx-4">
        <h2 class="text-2xl font-bold mb-6 text-center">Add a New Director</h2>
        <form method="POST" action="" class="space-y-4">
            <input type="text" name="name" placeholder="Director Name" class="w-full p-2 border border-gray-300 rounded" required>
    ...
    <div class="bg-white w-full max-w-4xl p-8 shadow-lg rounded-lg mx-4">
        <h2 class="text-2xl font-bold mb-6 text-center">All Directors</h2>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td class='py-2 px-4'><a href='edit_director.php?id=" . $row['id'] . "' class='text-blue-500 hover:text-blue-700'>Edit</a> | <a href='delete_director.php?id=" . $row['id'] . "' class='text-red-500 hover:text-red-700'>Delete</a></td>";
                        echo "</tr>";
                    }
                } else {
                    echo "<tr><td colspan='3' class='text-center py-4'>No directors found</td></tr>";
                }
                ?>
            </tbody>
        </table>
